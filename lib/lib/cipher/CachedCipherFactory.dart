...
class CachedCipherFactory implements CipherFactory {
  ...
  static String FUNCTION_SWAP1_PATTERN =
      r"\{var\s\w=\w\[0];\w\[0]=\w\[\w%\w.length];\w\[\w]=\w\}";
  static String FUNCTION_SWAP2_PATTERN =
  ...
  Future<Cipher> createCipher(var jsUrl) async {
    ...
    if (cipher == null) {
      ...
      Map<String, CipherFunction> transformFunctionsMap =
          getTransformFunctionsMap(va, js);
  ...
  CipherFunction mapFunction(String jsFunction) {
    for (MapEntry<RegExp, CipherFunction> entry
        in functionsEquivalentMap.entries) {
